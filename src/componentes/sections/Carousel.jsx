...
const Carousel = () => {
  ...
  const updateIndex = useCallback(
    ...
    [totalItems] // Só é recriada se totalItems mudar.
  );
  ...
  const getCardTransform = useCallback(
    (index) => {
      ...
      // Valores padrão.
      let x, scale, zIndex, rotateY, opacity, textOpacity, isClickable; // Se o cartão é clicável.
    ...
    <>
      <div className="carousel__container">
        <button
          ...
          onClick={goToPrev}
          aria-label="Voltar para o item anterior do carrossel"
        >
            ...
            {cardsCarousel.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  aria-label={
                    isClickable
                      ? `Visualizar detalhes ou navegar para a categoria de ${item.title}`
                      : `Item ${item.title} no carrossel`
                  }
                  tabIndex={isClickable ? 0 : -1} // Permite focar apenas se for clicável
                >
        ...
        <button
          ...
          onClick={goToNext}
          aria-label="Avançar para o próximo item do carrossel"
        >
